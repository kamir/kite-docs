---
layout: page
title: Calculate the Descriptive Dataset Statistics for Datasets
---
...
alt="Etosha-Stats Video" width="240" height="180" border="10" align="right" title="Link to Etosha-Stats Video"/></a>

Kite includes a toolset to handle the descriptive statistics on a per column base for datasets in Hadoop clusters. This way, some jobs do not have
to reprocess the dataset to gather such values, because such information already exists. Descriptive metadata can also be exposed to other applications
via an SPARQL endpoint, implemented on top of SOLR. 

This short tutorial walks you through the process of creating a dataset descriptor, called a "DOAD" file. You will publish the column statistics and query it locally.

## Preparation

Please start with downloading the Kite stats-tools Jar fiel. This jar is the executable that runs the mapreduce jobs. So save it as `kitestats.jar`. To download with curl, run:

```
curl http://central.maven.org/maven2/org/kitesdk/kite-tools/0.15.0/kite-tools-0.15.0-binary.jar -o kitestats.jar
chmod +x kitestats.jar
```

We go through the dataset creation procedure here. For more details please see: <a href="#">KITE TOOLS</a>
